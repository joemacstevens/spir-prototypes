...
| **Tap "Open Deep Breathing →" on Deep Breathing row** | Shows a small toast "In-app: opens the breathing module" — intentionally non-interactive in this prototype. Logging is **not** a manual time entry; minutes are tracked authoritatively from completed breathing cycles inside the existing breathing module ([`prototypes/breathing-module/`](../breathing-module/index.html)). | Routes into the existing `BreathworkPanel.cs` flow. The Deep Breathing satellite ring's `image.fillAmount` updates from cycles completed there (cycles × pattern duration → minutes), not from any new modal in `CheckInsPanel`. Reuse the existing 5-cycle XP refund mechanic Jay called out in his 2026-05-01 23:46 SMS — that mechanic is cycle-based and lives in the breathing module. |
...
- `BreathworkPanel.cs` — Deep Breathing row "Open Deep Breathing →" button routes here. The breathing module already logs cycles + awards XP (5/cycle-set free, 20 Pro per [`prototypes/breathing-module/CLAUDE.md`](../breathing-module/CLAUDE.md)) and owns the 5-cycle refund mechanic Jay specified.
