...
| 14 | **Deep Breathing mini-ring** (formerly Nap) | Currently a row with icon + Edit button | 60px satellite tile, ring is empty (greyed) when no deep-breathing logged, label shows "—" | Same `Image` Filled/Radial360 setup. When nothing logged: only the track ring is visible, `mini-ring-val` shows "—" in muted text. When logged: fill amount based on duration vs target. | Same `DOFillAmount` tween | `CheckInsPanel.cs`, reads existing nap data path from `DataConfig.checkinId` (server-fetched). **Label change only** — backend Unity field naming (`napValue`, etc.) unchanged in this spec; rename is product-facing. | No (existing data path) |
...
| 17 | **Deep Breathing edit row** (formerly Nap) | Existing — eye-shaped icon + "Nap" + "No nap logged" + Edit | Same icon (blue ellipse with two pupil dots), title "Deep Breathing", subtitle "No deep breathing logged", Edit button on right | `Image` (icon, 20×20) + `TMP_Text` × 2 (title + subtitle) + `Button` with rounded background. | None | `CheckInsPanel.cs`. Edit button still calls existing `AddNapTodayPopup.Show()` → `SaveDateAndTime()` → `NetworkAPImanager.AddNapToCheckIn()` → `GameManager.SpawnEffect(confettiBlue)`. **Product-facing rename only** in this pass; refactoring class names to `AddDeepBreathingPopup` is a future ticket. | No (existing flow) |
...
- `AddNapTodayPopup.cs` — Deep Breathing row Edit button (`AddNapTodayPopup.Show()`) — class rename out of scope for this ticket
