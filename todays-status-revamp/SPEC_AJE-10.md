...
| **Tap "Edit" on Nap row** | Opens an **in-phone bottom-sheet modal** (scoped to `.phone-screen`, not full viewport — backdrop dim and sheet are confined to the phone frame, like a native iOS sheet inside the device). Start/end time inputs + live duration calc. Save → row updates to "1h 30m · 1:00 PM → 2:30 PM" in blue, the Nap satellite ring fills proportionally to a 90-min target, and a green toast confirms (also in-phone). Cancel/backdrop/Esc closes. See [`05-nap-modal-in-phone.png`](./spec-screenshots/05-nap-modal-in-phone.png). | Existing `AddNapTodayPopup.Show()` flow already does the modal + `NetworkAPImanager.AddNapToCheckIn()` + `GameManager.SpawnEffect(confettiBlue)`. The new bit is updating the satellite ring's `image.fillAmount` after save (see element-table row 14). |
...
| 3 | "Energy Available" label | Existing label | Same — left-aligned, font weight 600, size 14px | `TMP_Text` | None | `CheckInsPanel.cs` | No |
...
| 8 | "Energy Consumed" label | Existing label or new | Same style as Energy Available label | `TMP_Text` | None | `CheckInsPanel.cs` | No |
...
| 14 | **Nap mini-ring** | Currently a row with icon + Edit button | 60px satellite tile, ring is empty (greyed) when no nap logged, label shows "—" | Same `Image` Filled/Radial360 setup. When no nap logged: only the track ring is visible, `mini-ring-val` shows "—" in muted text. When logged: fill amount based on nap duration vs target. | Same `DOFillAmount` tween | `CheckInsPanel.cs`, reads nap data from `DataConfig.checkinId` (server-fetched) | No (existing data path) |
| 15 | **Fasting mini-ring** | Currently a row with icon + Edit button | 60px satellite tile: purple ring at 100% fill, "14h" text | Same `Image` Filled/Radial360 setup, but stroke uses purple gradient (#5A49B8 → #9D8AFF). Drive `fillAmount = currentFastingHours / 14f` (or current fasting target). | Same `DOFillAmount` tween | `CheckInsPanel.cs`, reads `DataConfig.userTodaySurvey.fastingValue` (already `int`) | No |
| 16 | Compact edit card | Currently the recovery card (with hydration row at top + Nap row + Fasting row, all standard padding) | Card with `padding: 14px 18px` and `padding: 6px 0` on rows. Only contains **Nap row** and **Fasting row** — NO hydration row. | Same `Image` (card bg) + `VerticalLayoutGroup`, but with reduced padding values | None | `CheckInsPanel.cs` | No |
| 17 | **Nap edit row** | Existing — eye-shaped icon + "Nap" + "No nap logged" + Edit | Same icon (blue ellipse with two pupil dots), title "Nap", subtitle "No nap logged", Edit button on right | `Image` (icon, 20×20) + `TMP_Text` × 2 (title + subtitle) + `Button` with rounded background. | None | `CheckInsPanel.cs`. Edit button → `AddNapTodayPopup.Show()`. Confirm via `AddNapTodayPopup.SaveDateAndTime()` which already POSTs to `NetworkAPImanager.AddNapToCheckIn()` and triggers `GameManager.SpawnEffect(confettiBlue)`. | No (existing flow) |
| 18 | **Fasting Window edit row** | Existing — moon icon + "Fasting Window" + duration text + Edit | Same icon (purple crescent moon — note color change from blue), title "Fasting Window", subtitle "**14h · 8:00 PM → 10:00 AM**" *(note: "·" middot and "→" arrow — not "from … to …")*, Edit button | `Image` (moon icon, color `#7B68EE` purple) + `TMP_Text` × 2 + `Button` | None | `CheckInsPanel.cs`. Edit button → `FastingPopup.Show()` + `FastingPopup.Instance.EnableBGOpacity()` (existing pattern, see `CheckInsPanel.cs:1079`). | No (existing flow) |
| 19 | Success banner | Existing element | Card with green checkmark circle (32px, glow), text "Great day! Your energy, hydration, and recovery are all on track." | `Image` (banner bg) + `Image` (green circle) + `Image` (checkmark) + `TMP_Text` | Conditional show: `gameObject.SetActive(thresholds.AllMet)` — suggested defaults to pressure-test: First Wind ≥ 60%, Hydration ≥ 3/4, Fasting completed | `CheckInsPanel.cs` | No (computed locally from existing data) |
...
- Card label ("Energy Available"): 14px / 600 / `--text`
...
- `AddNapTodayPopup.cs` — Nap row Edit button (`AddNapTodayPopup.Show()`)
- `FastingPopup.cs` — Fasting row Edit button (`FastingPopup.Show()` + `EnableBGOpacity()`)
...
*Last updated: 2026-05-01 · Authored alongside [`index.html`](./index.html) merge in PR aje-10-merge-v1-v2-recovery.*
